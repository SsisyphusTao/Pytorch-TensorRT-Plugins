...
YoloDetectLayer::YoloDetectLayer(int num_cls, int max_det, std::vector<int> h, std::vector<int> w,
...
Dims YoloDetectLayer::getOutputDimensions(int index, const Dims* inputs, int nbInputs) noexcept
{
    ...
    if (index == 0)
        return Dims3(mMaxdet, 1, 4);
    else
        return Dims2(mMaxdet, mNumcls);
}
...
__global__ void Reshape(const float* input, float* loc, float* cof, int w, int h, int numClass, int mMaxdet, int stride,
    const float* anchors, int* countAddress)
{
    CUDA_KERNEL_LOOP(idx, h * w * NUM_ANCH)
    {
        ...
        if (input[(anchorPos * infoLen + 4) * mapSize + mapPos] < CONF_THRES)
            continue;
    ...
    int batchSize, const void* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    ...
    for (int i = 0; i < NUM_HEAD; i++)
    {
        ...
        Reshape<<<GET_BLOCKS_(mHeights[i] * mWidths[i] * NUM_ANCH), CUDA_NUM_THREADS, 0, stream>>>(
            static_cast<const float*>(inputs[i]), loc, cof, mWidths[i], mHeights[i], mNumcls, mMaxdet, mStrides[i],
            anchors, count);
    }
    cudaFree(count);
    return 0;
...
DataType YoloDetectLayer::getOutputDataType(
...
bool YoloDetectLayer::isOutputBroadcastAcrossBatch(
...
IPluginV2Ext* YoloPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
    ...
    YoloDetectLayer* obj = new YoloDetectLayer(num_cls, max_det, heights, widths, strides, &mAnchors);
    obj->setPluginNamespace(mNamespace.c_str());
    return obj;
}

IPluginV2Ext* YoloPluginCreator::deserializePlugin(
