...
# from Mbv3Yolo import get_pose_net
...
class Yolov5(object):
    ...
    def add_conv(self, input_tensor, out_channels, kernel=1, stride=1, padding=None, group=1, act=True, parent=''):
        ...
        if act:
            ...
            # ac.alpha = 0.1
            ac = self.network.add_activation(input=conv.get_output(0), type=trt.ActivationType.SIGMOID)
            return self.network.add_elementwise(ac.get_output(0), conv.get_output(0), trt.ElementWiseOperation.PROD)
        else:
            return conv
    ...
    def add_c3(self, input_tensor, out_channels, n=1, shortcut=True, g=1, e=0.5, parent=''):
        c_ = int(out_channels * e)
        m = self.add_conv(input_tensor, c_, parent=parent+'.cv1')
        cv2 = self.add_conv(input_tensor, c_, parent=parent+'.cv2')
        for i in range(n):
            m = self.add_bottleneck(m, c_, shortcut, g, 1.0, parent=parent+'.m.%d'%i)
        ct = self.network.add_concatenation([m.get_output(0), cv2.get_output(0)])
        cv3 = self.add_conv(ct, out_channels, parent=parent+'.cv3')
        return cv3
    ...
    def add_yoloHead(self, input_tensors):
        ...
        num_cls = trt.PluginField("num_cls", np.array([5], dtype=np.int32), trt.PluginFieldType.INT32)
    ...
    def add_nms(self, input_tensors):
        ...
        numClasses  = trt.PluginField("numClasses", np.array([5], dtype=np.int32), trt.PluginFieldType.INT32)
    ...
    def populate_network(self):
        ...
        L13 = self.add_c3(L12, 512, 3, False, parent='model.13')
    ...
    def build_engine(self):
        ...
        with trt.Builder(TRT_LOGGER) as builder, builder.create_network() as network, builder.create_builder_config() as config, trt.Runtime(TRT_LOGGER) as runtime:
...
def main():
    ...
    weights = torch.load('yolov5l.pth', map_location={'cuda:0':'cpu'})
    # Do inference with TensorRT.
    with Yolov5(weights).engine as engine:
        with open('yolov5.plan', "wb") as f:
            f.write(engine)
    with open('yolov5.plan', "rb") as f, trt.Runtime(TRT_LOGGER) as runtime:
